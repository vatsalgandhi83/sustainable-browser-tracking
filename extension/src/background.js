...
async function analyzeUrlAndNotify(urlString) {
  ...
  try {
    const data = await fetchWithRetry(`https://api.thegreenwebfoundation.org/api/v3/greencheck/${domain}`);
    isGreen = data.green;
  } catch (error) {
    console.error(`Could not analyze URL after multiple retries: ${urlString}`, error);
  }
...
function updateStats(isGreen) {
    ...
    chrome.storage.sync.get(["dailyStats"], (result) => {
        ...
        if (!stats.date || stats.date !== today) {
            stats = { date: today, co2: 0, greenVisits: 0, totalVisits: 0, points: 0, badges: [] };
        }
        ...
        if (isGreen) { stats.greenVisits += 1; }
